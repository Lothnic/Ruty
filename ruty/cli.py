...
def interactive_chat():
    ...
    # Use local SQLite database for persistence
    db_path = "ruty.sqlite"
    # SqliteSaver.from_conn_string expects a path, not a URI
    conn_string = str(Path(db_path).resolve())
    
    print(f"⚡ Connecting to local database: {conn_string}")
    ...
    with SqliteSaver.from_conn_string(conn_string) as checkpointer:
        ...
        print("\n🧠 Ruty - LangGraph Agent (Persistent)")
        print("=" * 40)
        print("Commands:")
        ...
        # Track messages for saving later
        ...
        try:
            while True:
                ...
                try:
                    ...
                    # Manual accumulation because we want to save purely what was shown in CLI
                    # (Though pulling from graph state on exit is more robust)
            ...
            # On exit loop (Ctrl+C or /quit), try to save history
            try:
